...
// let isoVersion = navigator.userAgent.match(/(iPhone\sOS)\s([\d_]+)/)[2]
//   .replace(/_/g, '.') // 取出ios版本号
let DB = {
  dbName: 'cmpDB',
  // slqVersion: isIOS && isoVersion.split('.')[0] > 9 ? '1.0' : '1', // websql的版本号，由于ios的问题，版本号的写法不一样
  slqVersion: '1', // websql的版本号，由于ios的问题，版本号的写法不一样
  ...
  save: function (key, value, callback) {
    // 保存数据到数据库  key---数据key  value----数据值  callback----回调函数
