  ...
  <div class="salesOrderDfh">
    <scroll ref="scroll" class="nominate-content"
      ...
      <p class="clr lh80 bdb pl3 pr3">
        ...
        <span class="clblue right fs28">待发货</span>
      </p>
      ...
      <div class=" pl3 pr3 lh60 clr bdb">
        ...
        <div class="flex flex-justify-space">
          ...
          <router-link :to="{path:'/checkEvidence',query:{orderId:item.id,type:type}}" class="fs22 clblue ">查看汇款凭证</router-link>
        </div>
      </div>
      <div class="pt30 pb40 pr3 tr">
        ...
        <a href="javascript:;" class="z-bluek inlineblock" @click="remindFh(item.id)">提醒发货</a>
      </div>
  ...
  export default {
    ...
    methods:{
      goToCommuction (data) {
        ...
        this.$router.replace({path:'/communication',query:{cid:data.id,companyImg:data.companylogo,companyName:data.company_name,tel:data.phone}})
      },
      scroll (pos) {
        this.scrollY = pos.y
