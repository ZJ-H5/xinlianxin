    ...
    <div>
      <div class="fixedtop">
        <div class="header">
          ...
          <span class="txt">修改发票信息</span>
          <a class="add" @click="savefp">保存</a>
        </div>
      </div>
      <div class="fptab clr">
        <router-link tag="div"  class="fptab-link" :to="{name:'editgeneralInvoice',params:{type:0}}" :name="name" replace>
          <p>普通发票</p>
        </router-link>
        <router-link tag="div" class="fptab-link"  :to="{name:'editvatInvoice'}" :name="name" replace>
          <p>增值税发票</p>
        </router-link>
      </div>
      <keep-alive>
      ...
      <toast position="middle" :value="deleteshow" :text="text"  width="50%"></toast>
    </div>
</template>
