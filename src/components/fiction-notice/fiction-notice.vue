<template>
  <div class="mt20">
    <scroll ref="scroll"
      ...
      :data="tranList">
      <div class="wrap">
        <div class="tranbox bgf p20" v-if="tranList.length" v-for="(item,index) in tranList" :key="index" @click="_updatamessage(item.id)">
          <div class="tranPage">
            <span class="c3 fs30">{{item.title}}</span>
            <span class="c9 right">{{item.create_time|dateFormats}}</span>
          </div>
          <div class="tranPage">
            <div class="fs28 c6" v-html="item.content"></div>
          </div>
        </div>
      </div>
    </scroll>
