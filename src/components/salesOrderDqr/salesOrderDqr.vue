    ...
    <div class="bgf mb20" v-for="(item,index) in list">
      <router-link :to="{path:'/orderdetailDqr',query:{orderId:item.id}}">
        ...
        <div class="pt30 pb40 pr3 tr">
          <router-link :to="{path:'/checkUploadFile',query:{contract:item.contract}}" class="z-link c6 inlineblock mr20 z-ckfp" v-if="item.contract!=''">查看合同附件</router-link>
          <commit-company :item="item" :type="type"></commit-company>
          <a href="javascript:;" class="z-bluek inlineblock" @click="confirmOrder(item.id,index)">确认订单</a>
        </div>
...
<script>
  ...
  export default {
    ...
    methods:{
        ...
        sellerOrderListdo({
          ...
          currentType:1
            ...
            if(res.code==1){if (res.data.list.length <= 0) {
              ...
              this.loadflag = false
              this.list.push(...(res.data.list))//销售待确认订单列表
      ...
      _sellerOrderDetailsdo(id,i){//详情页
