  ...
  <div>
    <scroll ref="scroll" class="nominate-content"
      ...
      <p class="clr pt20 pb20 bdb pl3 pr3 flex flex-align-center">
        ...
        <a href="javascript:;" class="z-bluek" @click="bj(item.id)">重新报价</a>
      </p>
    </div>
      </div>
    </scroll>
    ...
    <x-dialog :show="bjshow" @on-hide="cartDialog" @on-show="dialogShow">
      <div class="bgf2">
        <div class="bgf mb20">
    ...
    _myOfferListdo(){
      myOfferListdo({
        ...
        if(res.code==1){
          ...
          this.loadflag = false
          this.list.push(...(res.data.list)) //已忽略的列表
    ...
    _AppagainOfferdo(){
      AppagainOfferdo({
        ...
        if(res.code==1){
          ...
          this.offerUnit=res.data.quoted_price
